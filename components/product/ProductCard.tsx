...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/products/${product.slug}`}>
      <Card>
        {/* Product image area */}
        <div className="relative aspect-[4/3] bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center overflow-hidden rounded-t-[clamp(0.5rem,1.5vw,0.75rem)]">
          {hasRealImage ? (
            <Image
              src={imageUrl}
              alt={product.name}
